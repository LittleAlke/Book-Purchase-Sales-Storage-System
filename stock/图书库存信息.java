...
public class 图书库存信息 extends JPanel implements ActionListener {
	  ...
	  public  图书库存信息() {
        ...
        String sql3="select storeHouseNumber from storehouse ";
        model3 = new  DefaultTableModel(new Query(sql3).getData(),new Query(sql3).getColName());  
        table3 = new JTable(model3);
        int rows3 = table3.getRowCount();//获取表的行数
     System.out.print(rows3+"  ");
        String []s1 = new String[rows3];
        int k=-1;
        for (int j=0;j<rows3;j++) {
        	k++;
 			s1[k]=table3.getValueAt(j,0).toString();//读取你获取行号的某一列的值（也就是字段）
 	System.out.print(table3.getValueAt(j,0).toString()+"  ");
	...
	System.out.print(s1+"  ");
	System.out.print(cx.getSelectedItem()+"  ");
	 ...
	 System.out.print(table2.getRowCount()+"  ");
	        int rowz2;
	        if (26+rows2*31<508) {
	        	rowz2=(26+rows2*31);
			}else {rowz2=508;}    
